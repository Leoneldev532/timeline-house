window.addEventListener("DOMContentLoaded", () => {
  ...
  const titleRotator = createVerticalTextRotator(
    ".hero__title",
    carouselTitles,
  );
  const stepRotator = createVerticalTextRotator(
    ".hero__step-current",
    carouselSteps,
  );
  ...
  menuToggleBtn.addEventListener("click", () => {
    if (!isMenuOpen) {
      ...
      menuTimeline
        .to(menuLinks, { y: 50, stagger: 0.04, opacity: 0.4 }, ">")
        .to(
          menuContainer,
          {
  ...
  const progressValueElement = document.querySelector(
    ".hero__overlay-progress-value",
  );
  const overlayTimeline = gsap.timeline();

  gsap.set(".hero__overlay-panel--top", {
    clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)",
  });
  gsap.set(".hero__overlay-panel--bottom", {
    clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)",
  });
  ...
  overlayTimeline
    ...
    .to(
      ".hero__overlay-text",
      { y: 0, duration: 0.8, stagger: 0.05, ease: "sine" },
      "-=0.1",
    )
    .to(progressValueElement, { autoAlpha: 0 }, "<")
    .set(
      ".hero__overlay-progress-bar",
      { opacity: 0, duration: 0.5, ease: "linear" },
      ">",
    )
    .to(
      ".hero__overlay-panel--top",
      {
        clipPath: "polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)",
        duration: 1,
        ease: "expo.inOut",
